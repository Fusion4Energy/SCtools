...
class ATLAS:
    ...
    def write_page(self, i):
        """
        Creates a docx that consists on the template page fulfilled with a component 
            info.
        """
        ...
        if str(df_row["CELL IDs"]) == "nan":
            ...
            context = {
                ...
                "mass": "{:.2f}".format(
                    float(df_row["DENSITY [g/cm3]"])
                    * float(df_row["ORIGINAL VOLUME [cm3]"])
                ),  #'mass'        : df_row['MASS [g]'],
                "comment": "N/A",
            }
        pic_ori = InlineImage(
            page, Original_pictures + "\\" + str(i + 2) + ".jpg", width=Cm(11)
        )
